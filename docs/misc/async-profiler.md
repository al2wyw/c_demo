...
### 1. 启动阶段 `PerfEvents::start`（[perfEvents_linux.cpp:812](/Users/liyang/IdeaProjects/async-profiler/src/perfEvents_linux.cpp)）
...
- **信号选择**：`_signal = OS::getProfilingSignal(0)`，优先 `SIGPROF`，由 [os_linux.cpp:289](/Users/liyang/IdeaProjects/async-profiler/src/os_linux.cpp) 实现。
...
### 2. 单线程武装 `createForThread`（[perfEvents_linux.cpp:632](/Users/liyang/IdeaProjects/async-profiler/src/perfEvents_linux.cpp)）
...
### 4. 用户态信号处理器 `PerfEvents::signalHandler`（[perfEvents_linux.cpp:781](/Users/liyang/IdeaProjects/async-profiler/src/perfEvents_linux.cpp)）
...
### 5. 计数器读取与多路复用补偿 `readCounter`（[perfEvents_linux.cpp:743](/Users/liyang/IdeaProjects/async-profiler/src/perfEvents_linux.cpp)）
...
### 6. 调用栈采集 `PerfEvents::walk`（[perfEvents_linux.cpp:902](/Users/liyang/IdeaProjects/async-profiler/src/perfEvents_linux.cpp)）
...
### 7. 样本归集 `Profiler::recordSample`（[profiler.cpp:392](/Users/liyang/IdeaProjects/async-profiler/src/profiler.cpp)）
...
CTimer 是 async-profiler 在 Linux 上的"二号"CPU 采样引擎，源码集中在 [ctimer_linux.cpp](/Users/liyang/IdeaProjects/async-profiler/src/ctimer_linux.cpp)、[cpuEngine.cpp](/Users/liyang/IdeaProjects/async-profiler/src/cpuEngine.cpp)。它的本质可以一句话概括：
...
[ctimer_linux.cpp:72-114](/Users/liyang/IdeaProjects/async-profiler/src/ctimer_linux.cpp)
...
`setupThreadHook()` 定位到 libjvm/libazsys/libj9thr 的导入表项 `pthread_setspecific`，把它替换为 [`pthread_setspecific_hook`](/Users/liyang/IdeaProjects/async-profiler/src/cpuEngine.cpp)：
...
这是 CTimer 与其它引擎最本质的差异点。看 [ctimer_linux.cpp:21-59](/Users/liyang/IdeaProjects/async-profiler/src/ctimer_linux.cpp)：
...
`createForAllThreads()`（[cpuEngine.cpp:95](/Users/liyang/IdeaProjects/async-profiler/src/cpuEngine.cpp)）通过 `OS::listThreads()` 遍历 `/proc/self/task`，对每个 tid 调一次 `createForThread`。新生线程则由前面提到的 `pthread_setspecific_hook → onThreadStart` 兜住——两者合在一起保证不漏线程。
