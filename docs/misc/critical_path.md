...
```c
while (ecx != edx) {
    *rsi = eax;          // 把上一轮算好的 eax 写回内存
    eax  = *rdi;         // 从内存重新读 eax
    ecx += 1;
    eax += 1;            // eax 自增（覆盖循环不变量场景）
}
```
...
```
┌──────────────────────────────────────────────────────────────┐
│  关键路径 = max(A 链, B 链) = max(5~6, 1) = 5~6 cycle/iter   │
└──────────────────────────────────────────────────────────────┘
```
...
```
 ...
 store eax,[addr]                                          ┐
       │                                                   │
       │ STLF (4~5 cyc)                                    │ A 链 ≈ 5~6 cyc
       ▼                                                   │ ⬅ 关键路径
 load  [addr]→eax  ──► add $1,%eax  ──► store eax,[addr]  ┘
                                              │
                                           （进入 i+1）

 add $1,%ecx (1c) ──► add $1,%ecx (1c) ──► add $1,%ecx (1c)   B 链 = 1 cyc
       │                  │                  │
      cmp/jne            cmp/jne            cmp/jne          (旁支)
```
...
.L3:
    movl    %eax, (%rsi)     ; (1) Store: [rsi] ← eax
    movl    (%rdi), %eax     ; (2) Load:  eax ← [rdi]   ← 地址不同！
    addl    $1, %ecx         ; (3) ecx++
    addl    $1, %eax         ; (4) eax++
    cmpl    %ecx, %edx       ; (5) flags = edx - ecx
    jne     .L3              ; (6) 分支
```

**关键前提变化**：`rsi ≠ rdi` 且循环内两个指针都不变 →
- `[rsi]` 是循环的"输出地址"（每轮被覆盖写）
- `[rdi]` 是循环的"输入地址"（每轮读到的是**同一个值**，循环不变量）
...
Cycle:        0    1    2    3    4    5    6    7
              ─    ─    ─    ─    ─    ─    ─    ─
迭代 0:   ┌─ store[rsi] ─┐
          │  load[rdi]   │── add eax(完成于 cyc≈5，因 load 延迟 ~4)
          │  add ecx ────│──► (循环携带，给迭代1)
          │  cmp/jne     │
          └──────────────┘

迭代 1:        ┌─ store[rsi] ─┐    （load/store 与迭代 0 并行执行！）
               │  load[rdi]   │
               │  add ecx ────│──► (给迭代 2)
               │  cmp/jne     │
               └──────────────┘
...
> ⚠️ load 自身延迟 ~4~5 cyc 不是问题，因为它**不在循环携带链上**，OoO 引擎会用 Reservation Station 把多轮 load 排队并行发射。
...
🔑 **当 `rsi ≠ rdi`（且无 4K aliasing）时**：

1. **关键路径剧变**：从 `store→STLF→load→add`（5~6 cyc）→ 仅剩 `add $1,%ecx`（**1 cyc**）
2. **真正的循环携带依赖只剩 ECX 计数器**，eax 链被 load 每轮"重置"，store 是纯输出
3. **瓶颈从延迟（latency-bound）转为吞吐（throughput-bound）**，受前端发射宽度和 µop 数限制
4. **吞吐量 ≈ 1 cycle/iter，IPC ≈ 5**，相比 `rsi == rdi` 场景**性能提升约 5 倍**
