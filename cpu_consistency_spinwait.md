...
pthread_barrier_t start_barrier;  // 每轮起跑屏障
pthread_barrier_t end_barrier;    // 每轮结束屏障

void* workerA(void* arg) {
    int count = 0, fail = 0;
    for (int i = 0; i < LOOP_COUNT; i++) {
        a = 0;                                          // 先清零自己的标志
        pthread_barrier_wait(&start_barrier);           // 同时起跑

        a = 1;
        __atomic_thread_fence(__ATOMIC_SEQ_CST);
        if (b == 0) count++; else fail++;

        pthread_barrier_wait(&end_barrier);             // 同时收尾，进入下一轮
    }
...
int main(int argc, char *argv[]) {
    ...
    pthread_barrier_init(&start_barrier, NULL, NUM);
    pthread_barrier_init(&end_barrier,   NULL, NUM);
...
用两个 flag，每个线程**只写自己的 flag、只读对方的 flag**，形成严格交替。

### 核心思路

每轮做"四步握手"：
1. A 把 `a_done = 0`，B 把 `b_done = 0`（重置）
2. A、B 各自做事
3. A 设 `a_done = 1`，等 `b_done == 1`
4. 双方都看到对方完成 → 进入下一轮
...
volatile int a = 0;
volatile int b = 0;
volatile int a_ready = 0;  // A 本轮是否完成
volatile int b_ready = 0;  // B 本轮是否完成
volatile int round_a = 0;  // 当前轮次（A 视角）
volatile int round_b = 0;
...
void* workerA(void* arg) {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
        ...
        while (__atomic_load_n(&b_ready, __ATOMIC_ACQUIRE) == 0) ;

        // 3. 由 A 统一推进轮次（约定 A 当裁判）
        __atomic_store_n(&b_ready, 0, __ATOMIC_RELAXED);
        __atomic_store_n(&a_ready, 0, __ATOMIC_RELAXED);
        __atomic_store_n(&round_b, i + 1, __ATOMIC_RELAXED);
        __atomic_store_n(&round_a, i + 1, __ATOMIC_RELEASE);
    }
    printf("workerA finished: %d, fail: %d\n", count, fail);
    return NULL;
}
...
void* workerB(void* arg) {
    ...
    for (int i = 0; i < LOOP_COUNT; i++) {
        ...
        __atomic_store_n(&b_ready, 1, __ATOMIC_RELEASE);
        // B 不推进轮次，等 A 推进
        while (__atomic_load_n(&round_b, __ATOMIC_ACQUIRE) != i + 1) ;
    }
    printf("workerB finished: %d, fail: %d\n", count, fail);
    return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    pthread_t threads[NUM];
    pthread_create(&threads[0], NULL, workerA, NULL);
    pthread_create(&threads[1], NULL, workerB, NULL);
...
我来把 [cpu_consistency.c](/Users/liyang/IdeaProjects/c_demo/cpu_consistency.c) 改写成"双 flag 严格乒乓"的版本。这种方式不引入额外的 fence/锁，最适合观察 Store-Load 重排现象。
...
```c
sleep(1);
__atomic_store_n(&round_no, 0, __ATOMIC_RELEASE);
```

`sleep(1)` 后两线程已经卡在 `while (round_no != 0)`，main 写入 0 的那一刻**两线程几乎同时通过 while**，进入并发执行段——这正是观察重排所需的"同时起跑"条件。

### 6. 每轮收尾时清零 `a` 和 `b`

由 A 在第 ④ 步统一清零（B 看不见 a/b 的写入，只观察 `done` 标志即可推进）。**避免上一轮残留的 `a=1` / `b=1` 污染下一轮的 `if (b == 0)` 判断**。

## 编译运行建议

```bash
gcc -O2 -pthread -o cpu_consistency cpu_consistency.c
...
| 职责 | 旧版（A 当裁判） | 新版（main 当裁判） |
|------|----------------|-------------------|
| 鸣枪 | main 写 0；之后 A 在每轮末尾推进 | main 在每轮开头写 i |
| 等待对方 | A 等 b_done，B 等 a_done | A、B 都不等对方，只举手就走 |
| 重置 a/b/done | A 在每轮末尾做 | main 在每轮开头做 |
| A、B 是否对称 | ❌ 不对称 | ✅ 完全对称 |
...
1. **A、B 完全对称** —— 实验对象的代码路径一模一样，更纯粹
2. **每轮 main 精确鸣枪** —— 起跑同步性更强（不再依赖 A 提前一拍写 round_no）
3. **可观测性更好** —— 想插入计时、统计、日志，全在 main 里加即可，不污染 worker
4. **方便扩展到 N 个线程** —— main 只要等 N 个 done 标志即可，对所有 worker 一视同仁
