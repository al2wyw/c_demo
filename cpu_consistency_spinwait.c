...
/*
untaken=0的原因:
硬件上（尤其是 x86 TSO），SC fence 会强制刷 store buffer，使得"两个写都已完成、两个读才开始"这种顺序的窗口极其狭窄
同时，A、B 的"赢跑"严重不均（98703 vs 1297），让两者的 Wa/Wb 几乎不可能"同时完成于双方读之前"
解决方法:
保持 SC，但显著拉大循环数到上亿次，并把两个线程绑到不同物理核心
或者在 ARM64 上跑（弱内存模型下窗口大得多）
 */