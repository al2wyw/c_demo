 ...
 * 跳转: (jmp/call类似)
 * 直接跳转: jmp 0x23af (跳转到addr)
 * 间接跳转: jmp *%rax or jmp *0x10(%rax) / jmp *0x23af (跳转到的addr从寄存器或内存中读出)
 *
 ...
 *  r; c; finish(fi); return; util; si; ni; jump; p/x ptr; p * ptr; set x=1; set {int}0x7fffffffe31c=13; display $rsp; ptype /m /o; info local/arg/reg/fun/addr/b xxxx; handle SIGSEGV nostop; set substitute-path /home /data/home
